...
float getPatternWeight(vec3 position) {
    ...
    // vec2 frac_xz = fract(position.xz);
    vec2 frac_xz = fract(position.xz / 2.0); // Double grid line distance
...
float getDistanceSceneTransparent(vec3 position, bool saveHit, inout HitInfo hInfo, float clock_val, vec3 target_val) {
    vec3 local = position;
    local.xyz = position.xyz - vec3(0.0, 3.0, 0.0);
    float field = getDistanceBox(local, vec3(1.0, 3.0, 1.0), 0.01);
...
vec3 computeLight(vec3 position, vec3 surfaceNormal, Material material, vec3 viewer_val, vec3 target_val, float clock_val) {
    ...
    vec3 lightColor    = 100.0 * vec3(1.0, 1.0, 1.0);
...
vec3 getSkyColor(vec3 ray_dir) {
    ...
    vec3 sky_bottom = vec3(0.0, 0.0, 0.0);
...
void main() {
    ...
    vec3 fixed_cube_target = vec3(0.0, 0.5, 0.0);
